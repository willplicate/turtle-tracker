...
const BS_IV         = 0.26;   // LEAPS IV — recalibrated for AAPL (modest reduction from IWM's 0.27); see CLAUDE.md
const BS_SHORT_IV_BASE = 0.22;   // baseline weekly-call IV at "calm" realized vol (AAPL_WEEKLY_DATA vix ~= BS_SHORT_IV_BASELINE_VIX)
const BS_SHORT_IV_BASELINE_VIX = 22; // the vix-equivalent reading BS_SHORT_IV_BASE is calibrated against
...
class ShortCallPosition {
    constructor(stockPrice, strike, dte = 7, contracts = 1, ivOverride = null) {
        this.strike = strike;
        this.dte = dte;
        this.contracts = contracts;
        this.openingStockPrice = stockPrice;
        this.iv = ivOverride ?? BS_SHORT_IV_BASE;
        this.premiumCollected = _bsCall(stockPrice, strike, dte * TRADING_TO_CAL / 365, BS_R, this.iv).price * contracts;
    ...
    advanceDay(newStockPrice) {
        ...
        if (this.dte === 0) {
            ...
            this.optionValue = _bsCall(newStockPrice, this.strike, this.dte * TRADING_TO_CAL / 365, BS_R, this.iv).price * this.contracts;
        }
...
class SharesPosition {
    constructor(stockPrice, contracts = 1) {
        this.kind = 'shares';
        this.shares = contracts * 100;
        this.contracts = contracts;
        this.costBasis = stockPrice * this.shares;
        this.value = this.costBasis;
        this.entryPrice = stockPrice;
        this.strike = null;   // no strike — health-alert/roll call sites use this to detect shares mode
        this.delta = 1.0;     // for any display code that reads .delta generically
        this.dte = null;      // no expiry — health-alert/roll call sites use this to detect shares mode
        this.theta = 0;
        this.intrinsic = this.value;
        this.extrinsic = 0;
    }

    advanceDay(newStockPrice) {
        const oldValue = this.value;
        this.value = newStockPrice * this.shares;
        this.intrinsic = this.value;
        return { value: this.value, delta: this.delta, theta: this.theta, dailyPnl: this.value - oldValue };
    }
...
class TradingSimulator {
    ...
    openShares(contracts = 1) {
        if (this.leapsPosition) return { error: 'Position already exists' };

        const shares = new SharesPosition(this.currentStockPrice, contracts);

        if (shares.costBasis > this.cash) {
            return { error: 'Insufficient cash', required: shares.costBasis, available: this.cash };
        }

        this.cash -= shares.costBasis;
        this.leapsPosition = shares;

        this.tradeHistory.push({
            day: this.currentDay, week: this.weekNumber,
            action: 'OPEN_SHARES', cost: shares.costBasis,
            stockPrice: this.currentStockPrice
        });

        return { success: true, cost: shares.costBasis, shares: shares.shares };
    }

    sellWeeklyCall(strike, dte = 7, contracts = 1, ivOverride = null) {
        ...
        const shortCall = new ShortCallPosition(this.currentStockPrice, strike, dte, contracts, ivOverride);
    ...
    getStatus() {
        ...
        if (this.leapsPosition) {
            ...
            status.leaps = { kind: l.kind, strike: l.strike, dte: l.dte, value: l.value, delta: l.delta, theta: l.theta, intrinsic: l.intrinsic, extrinsic: l.extrinsic, pnl: l.getPnl(), costBasis: l.costBasis };
        }
