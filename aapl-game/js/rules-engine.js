...
function getRulesSignal(vixHistory, priceHistory) {
    ...
    if (droppedFromPeak >= 8 && emaFalling && currentRVX < 20) {
        return { action: 'Go uncovered', reason: 'Volatility calm and falling — let the position run', colour: 'blue' };
    }
...
function generateStrikeOptions(currentPrice, currentVix, simulator, contracts) {
    ...
    function getPremium(strike) {
        ...
        if (simulator._pricingMode === 'advanced' && simulator._vixSim) {
            const call = new AdvancedShortCall(currentPrice, strike, 5, currentVix, BS_SHORT_IV_BASELINE_VIX);
            return call.premiumCollected * (contracts || 1);
        }
        const ivOverride = BS_SHORT_IV_BASE * (currentVix / BS_SHORT_IV_BASELINE_VIX);
        const call = new ShortCallPosition(currentPrice, strike, 5, 1, ivOverride);
        return call.premiumCollected * (contracts || 1);
    }
    ...
    return [
        ...
        { strike: null,           type: 'Uncovered', note: `No income — full ${simulator.leapsPosition?.kind === 'shares' ? 'share' : 'LEAPS'} participation`, premium: 0 },
    ];
}
